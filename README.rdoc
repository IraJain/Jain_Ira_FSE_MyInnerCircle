...
3 db migrations used to build the database incrementally.
...
2. paul_mccartney@test.com/hello123


Testing
========
To run tests : Go to Tools, Run rake task, Select specs.
This will run all the tests in spec folder
...
2.For browser verification use address :
3. Rspec test for XML in REST api - Postings->  postings_controller_spec under specification "describe "GET show" "

AJAX request:
=============
Implemented by dynamicaly rendering friends listing in home page without refreshing the whole page.
Refer to  AJAX request in views/friends/form which calls the 'add friend' form with remote: true.
Friends controller recieves this AJAX request and calls views/friends/create.js.erb to render partial index.html.erb
